...
/* Includes ------------------------------------------------------------------*/
#include <stdio.h>
#include "STM32F4_SDIO_SD_Driver.h"
#include "os.h"


OS_SEM  SD_Sem;			//SD卡入写或者读取等待信号量，中断发送信号量
...
/** 
  ...
  */
#define NULL 0
#define SDIO_STATIC_FLAGS               ((uint32_t)0x000007FF)
...
__IO uint32_t TransferEnd = 0, DMAEndOfTransfer = 0;		//用于标志传输是否结束，在中断服务函数中调用
...
SD_Error SD_Init(void)
{
	...
	errorstatus = SD_InitializeCards();	//初始化所有的卡或者单个卡进入就绪状态
	...
	/*上电识别，卡初始化都完成后，进入数据传输模式，提高读写速度速度若超过24M要进入bypass模式	*/
...
SD_Error SD_PowerON(void)
{
	...
	SDIO_CmdInitStructure.SDIO_Wait 	= SDIO_Wait_No;			//则CPSM在开始发送命令之前等待数据传输结束。 
	SDIO_CmdInitStructure.SDIO_CPSM 	= SDIO_CPSM_Enable;		//写命令进命令寄存器
...
SD_Error SD_GetCardInfo(SD_CardInfo *cardinfo)
{
	...
	cardinfo->SectorSize = 1<<cardinfo->SD_csd.RdBlockLen;
	cardinfo->SectorCount= cardinfo->CardCapacity/cardinfo->SectorSize;
...
SD_Error SD_ReadBlock(uint8_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize)
{
	...
	OSSemCreate(&SD_Sem,"SD Sem",(OS_SEM_CTR)0,&err);			//创建信号量	
	
	SDIO->ICR = SDIO_STATIC_FLAGS;
	SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND | SDIO_IT_RXOVERR | SDIO_IT_STBITERR, ENABLE);
	SDIO_DMACmd(ENABLE);
	STM32F4_SDIO_DMA_Receive((uint32_t *)readbuff, BlockSize);
	
	OSSemPend(&SD_Sem,2000,OS_OPT_PEND_BLOCKING,&ts,&err);		//等待信号量，超时时间为2000ms
	OSSemDel(&SD_Sem,OS_OPT_DEL_ALWAYS,&err);					//删除信号量
	
	if(SDIO->STA & SDIO_FLAG_DATAEND)
	{
		errorstatus = SD_OK;	
	}
	else
	...
	SDIO->ICR = SDIO_STATIC_FLAGS;								//清除状态标志位
#endif
	time = 0;
	while((SD_GetStatus() != SD_TRANSFER_OK)&&(time<500))
	{	
		OSTimeDly(1,OS_OPT_TIME_DLY,&err);	
		time++;
	} 
	if(time>=500) errorstatus = SD_ERROR;		
...
SD_Error SD_ReadMultiBlocks(uint8_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
	...
	SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND | SDIO_IT_RXOVERR | SDIO_IT_STBITERR, ENABLE);
	SDIO_DMACmd(ENABLE);
	STM32F4_SDIO_DMA_Receive((uint32_t *)readbuff, (NumberOfBlocks * BlockSize));
...
SD_Error SD_WriteBlock(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize)
{
	...
	SDIO_DMACmd(ENABLE);
	STM32F4_SDIO_DMA_Send((uint32_t *)writebuff, BlockSize);
...
SD_Error SD_WriteMultiBlocks(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
	...
	SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND | SDIO_IT_TXUNDERR | SDIO_IT_STBITERR, ENABLE);
	SDIO_DMACmd(ENABLE);
	STM32F4_SDIO_DMA_Send((uint32_t *)writebuff, (NumberOfBlocks * BlockSize));
...
{ 
	OS_ERR err;
	OSSemPost(&SD_Sem,OS_OPT_POST_1,&err);
